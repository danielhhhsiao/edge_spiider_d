...
$(document).on("click","#ftpTest",function(){
    var ip = $("#ftpIp").val();
    var name = $("#ftpUsername").val();
    var pw = $("#ftpUserpw").val();
...
$(document).on("click","#mqttTest",function(){
    var ip = $("#sgIp").val();
    var port = $("#sgPort").val();
    var name = $("#sgUser").val();
    var pw = $("#sgUserpwView").val();
    showLoading();
    var settings = {
        "url": _api_sysSetting,
        "type": "PUT",
    ...
    $.ajax(settings).success(function (response) {
        console.log(response);
        if(response.state!=200){
            alert("MQTT setting req error: "+response.msg);
            //alert(_language.system.FTP_err+response.msg);
        }
        else{
            //alert("FTP Setting successfully.");
            alert(response.mqtt_test);
        }
        hideLoading();
    }).error(function(e){
        //alert("FTP setting error." + e.statusText);
        console.log('ERROR');
        alert(_language.system.smart_grid_err + e.statusText);
        hideLoading();
    });
});
