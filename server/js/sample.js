    ...
    if(offset==0)
        ...
        for(var i=0;i<_ruleHeight.length;i++){
            if(scroll>=_ruleHeight[i].top-screenHalfH){
                $(".bottomInfo").html(_ruleHeight[i].name);
            }
        }
    }
...
function creatSensorSetting(type,name,bus,rate,range,source,sensorType,maxValue,minValue,biasValue,unit,fsUseVal){
    ...
    var titleDivHtml="Sample source:";
    if(type=="voltageInput")
        titleDivHtml+="Voltage input";
    if(type=="currentInput")
        titleDivHtml+="Current input";
    if(type=="vibration")
        titleDivHtml+="Vibration";
    if(type=="distance")
        titleDivHtml+="Distance";
    if(type=="digitalInput")
        titleDivHtml+="Digital input";
    if(type=="prognosis")
        titleDivHtml+="Vibration(Prognosis)";
    if(type=="audio")
        titleDivHtml+="Audio";
    ...
    if(type=="voltageInput" || type=="currentInput" || type=="vibration"){
        ...
        if(rate)
            _rate=rate;
        insertSubTitle(mainDiv,"Select bus (auto):",false);
        ...
        if(type=="voltageInput" || type=="currentInput"){
            insertSubTitle(mainDiv,"Sampling rate: ",false);
            insertSelectList(mainDiv,
                "sensorFs",
    ...
    insertSubTitle(mainDiv,"Channel setting:",true);
    ...
    if(type == "voltageInput"){
        ...
        insertDashed(mainDiv);
        insertSubTitle(mainDiv,"Please follow signal range background color to set the jumper:",true);
        ...
        insertDashed(mainDiv);
        insertSubTitle(mainDiv,"Please follow DC24V / DC12V type to set jumper:",true);
    ...
    insertDashed(mainDiv);
    insertSubTitle(mainDiv,"Remove this sensor:",true);
    insertCenterBtn(mainDiv,"btn sensorRemove removeBtn","Remove");
...
function creatSegSetting(name,
    ...
    if(rule == "0" ){
        insertSubTitle(mainDiv,"Rule: Normal");
        insertImg(mainDiv,"/example/segNormal.png");
    }
    else if(rule == "1"){
        insertSubTitle(mainDiv,"Rule: By strength");
        insertImg(mainDiv,"/example/segStrength.png");
    }
    else if(rule == "2"){
        insertSubTitle(mainDiv,"Rule: By Frequency.");
        insertImg(mainDiv,"/example/segFreq.png");
    }
    else if(rule == "3"){
        insertSubTitle(mainDiv,"Rule: Smar grid.");
        insertImg(mainDiv,"/example/segNormal.png");
        SGclass = "SGdiable";
        SGflag=false;
    }
    
    insertSubTitle(mainDiv,"Rule Name:",true);
    insertInputBox(mainDiv,"input_1_1 segRuleName",name);
    insertSubTitle(mainDiv,"Unit Name: (allow empty)",true,SGclass);
    insertInputBox(mainDiv,"input_1_1 segRuleUnit "+SGclass,unit);
    insertSubTitle(mainDiv,"Subunit Name: (allow empty)",true,SGclass);
    insertInputBox(mainDiv,"input_1_1 segRuleSubunit "+SGclass,sub_unit);
    if(rule != "0" && rule != "3")
        insertSubTitle(mainDiv,"Input item:",true);
        
    insertSelectList(mainDiv,"SegSource "+SGclass,[],[],false,"","SegSource segSelectList");
    if(rule == "0" || rule == "3")
        $(mainDiv).children(".segSelectList").hide();

    insertSubTitle(mainDiv,"Output item:",true);
    insertSubTitle(mainDiv,"(Click item to enable 'blue')",true);
    ...
    */
    ...
    if(rule != "0" && rule != "3" ){
        insertImg(mainDiv,"/example/signalOperation.png");
         insertSubTitle(mainDiv,"Window size(0.1~600 sec):",true);
    }
    else{
        insertSubTitle(mainDiv,"Capture length(0.1~600 sec):",true);
    }
    if(rule == "3")
    ...
    insertSubTitle(mainDiv,"Ignore length(0~N sec):",true);
    ...
    if(rule != "0" && rule != "3"){
        ...
        insertSubTitle(mainDiv,"Segmentation Threshold:",true);
        insertInputBox(mainDiv,"input_1_1 segThreshold","0");
        insertSubTitle(mainDiv,"Start point shift(-10~10 sec):",true);
        insertInputBox(mainDiv,"input_1_1 segShift","0");
        
        if(rule == "1"){ //segTime.
            insertSubTitle(mainDiv,"Capture length:",true);
            insertRadio(mainDiv,"leftLabel","subOperation",
                ["No","Yes"],
                ["Use window size.","Manual setting"],true);
                
            subMainDiv= insertDisplayBox(mainDiv);
            insertSubTitle(subMainDiv,"Length(0.01~900 sec):",true);
        ...
        if(rule == "2"){ //segFreq.
            ...
            insertSubTitle(mainDiv,"Target frequency:",true);
            insertRadio(mainDiv,"leftLabel","centerFrequencySelect",
                ["No","Yes"],
                ["Auto settings","Manual settings (1~Fs/2 Hz)"],true);
            subMainDiv= insertDisplayBox(mainDiv);
            ...
            insertRadio(mainDiv,"leftLabel","subOperation",
                ["No","Yes"],
                ["Use auto size.","Manual setting"],true);
        ...
        var signalFilterSwitch = insertRadio(mainDiv,"leftLabel","signalFilter",
            ["No","Yes"],
            ["Disable","Enable"],true,true,"Signal filter (95~105%):");
        console.log(signalFilterSwitch);
        subMainDiv= insertDisplayBox(mainDiv);
        insertSubTitle(subMainDiv,"Condition(0.01~N sec):",true);
        ...
        if(rule == "1"){
            ...
            var useFilterExceptionSwitch = insertRadio(subMainDiv,"leftLabel","useFilterException",
                ["No","Yes"],
                ["No","Yes"],true,true,"Use exception data between 5% to 95% of filter size:");
        }
    ...
    var uploadResultSegSwitch = insertRadio(mainDiv,"leftLabel "+SGclass,"uploadResultSeg",
        ["No","Yes"],
        ["No","Yes"],SGflag,true,"Upload segmentation result:");
    subMainDiv= insertDisplayBox(mainDiv,SGclass);
    insertSubTitle(subMainDiv,"Upload interval:(0~N)",true,SGclass);
    insertSubTitle(subMainDiv,"(When Feature extraction enable)",true,SGclass);
    insertInputBox(subMainDiv,"input_1_1 uploadResultSegInterval "+SGclass,"0");


    insertDashed(mainDiv,SGclass);
    insertSubTitle(mainDiv,"Feature extraction project:",true,SGclass);
    /*
    ...
    */
    ...
    var uploadResultFESwitch = insertRadio(mainDiv,"leftLabel "+SGclass,"uploadResultFE",
        ["No","Yes"],
        ["No","Yes"],SGflag,true,"Upload feature extraction result:");
    ...
    insertSubTitle(subMainDiv,"Format upload:",false);
    insertBtn(subMainDiv,"btn featureSelectBtn","Select file");
    
    insertDashed(mainDiv);
    insertSubTitle(mainDiv,"Remove this setting:",true);
    insertCenterBtn(mainDiv,"btn segRemove removeBtn","Remove");
...
function insertSensorSelectBox(main,
    ...
    label.innerHTML+="Enable "+enableName;
    ...
    insertSubTitle(heddened,"Name: ",false);
    ...
    if(type){
        ...
        insertSubTitle(typeHeddened,"Signal type: ",false);
    ...
    if(maxValue){
        ...
        insertSubTitle(typeHeddened,"Engineering upper limit : ",false);
    ...
    if(minValue){
        var typeHeddened=document.createElement("div");
        typeHeddened.className="typeHeddenedCT";
        insertSubTitle(typeHeddened,"Engineering lower limit : ",false);
    ...
    if(biasValue){
        var typeHeddened=document.createElement("div");
        typeHeddened.className="typeHeddenedCT";
        insertSubTitle(typeHeddened,"Engineering value offset : ",false);
    ...
    if(unit){
        var typeHeddened=document.createElement("div");
        typeHeddened.className="typeHeddenedCT";
        insertSubTitle(typeHeddened,"Engineering unit : ",false);
    ...
    if(d_s_remain){
        insertSubTitle(heddened,"(Need to switch jumper on board)",true);
    }
...
function insertSelectList(main,name,list,value,is1_2,useValue,otherClassName){
    var select=document.createElement("select");
    select.setAttribute("data-value",useValue);
    select.name=name;
    select.className="selectList";
    if(is1_2)
        select.className+="_1_2";
    if(otherClassName)
...
function waitTestStates(callback){
    ...
    $.ajax(settings).success(function (response) {
        if(response.state!=200){
            alert("wait test state change req error: "+response.msg);
            if(typeof(callback)==="function")
                callback();
        }
        else{
            if(response.data.environment.sensor_test_state=="1"){
                ...
                var enter="\n";
                var alertWord = "Upload result (success/total) "+enter;
                alertWord += "Seg. csv file was sent by FTP : "+response.data.environment.upload_ftp_seg_csv+enter;
                alertWord += "Seg. binary file was sent by FTP : "+response.data.environment.upload_ftp_seg_pkl+enter;
                alertWord += "Seg. result was sent by API : "+response.data.environment.upload_api_seg+enter;
                alertWord += "FE csv file was sent by FTP : "+response.data.environment.upload_ftp_fe_csv+enter;
                alertWord += "FE binary file was sent by FTP : "+response.data.environment.upload_ftp_fe_pkl+enter;
                alertWord += "FE result was sent by API : "+response.data.environment.upload_api_fe+enter;
                alertWord += "Smart grid was sent by MQTT : "+response.data.environment.upload_mqtt;
                console.log(alertWord);
                alert(alertWord);
                
                enter="</br>";
                var textResult = "Message(success/total)"+enter
                textResult += enter
                textResult += "Segmentation:"+enter
                textResult += ". . . FTP(csv) = "+response.data.environment.upload_ftp_seg_csv+enter;
                textResult += ". . . FTP(binary) = "+response.data.environment.upload_ftp_seg_pkl+enter;
                textResult += ". . . Web API = "+response.data.environment.upload_api_seg+enter;
                textResult += enter
                textResult += "Feature Extraction:"+enter
                textResult += ". . . FTP(csv) = "+response.data.environment.upload_ftp_fe_csv+enter;
                textResult += ". . . FTP(binary) = "+response.data.environment.upload_ftp_fe_pkl+enter;
                textResult += ". . . Web API = "+response.data.environment.upload_api_fe+enter;
                textResult += enter
                textResult += "Smart grid:"+enter
                textResult += ". . . MQTT = "+response.data.environment.upload_mqtt;
...
function loadSetting(async,name,path,callback){
    ...
    $.ajax(settings).success(function (response) {
        if(response.state!=200){
            alert("Load "+name+" setting req error: "+response.msg);
            if(typeof(callback)==="function")
                callback(null);
        }
        else{
        ...
        alert("Load "+name+" setting req error." + e.statusText);
        if(typeof(callback)==="function")
            callback(null);
    });
}
    ...
    if(type=="sensor"){
        ...
        */
        ...
        for(var i=0;i<sensorSetting.length;i++){
            ...
            if(!isNaN(busIndex) && _sensorList.bus[busIndex].name!=sensorType){
            ...
            else if(sensorType=="distance" && _sensorList.distance==false){
    ...
    if(type=="sensor" || type=="channel"){
        ...
        for(var i=0;i<checkbox.length;i++){
            let checkList = [
                ...
                ".sensorChUnit"];
            let emptyAlermList = [
                "The name of the channel is empty.",
                "The max value of the channel is empty.",
                "The min value of the channel is empty.",
                "The bias value of the channel is empty.",
                "The unit of the channel is empty."];
            let detectAlermList = [
                "The name of the channel is format error.",
                "The max value of the channel is format error.",
                "The min value of the channel is format error.",
                "The bias value of the channel is format error.",
                "The unit of the channel is format error."];
