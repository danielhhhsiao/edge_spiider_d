...
$(document).on("click",".defaultFeatureBtn",function(){
    if(basefreq = prompt(_language.sample.seg_default_FE_baseFreq)){
        if (!isInt(basefreq)) { 
...
function segRuleLoad(obj,form,sensor_form,useDefault,segCnt){
    ...
    if(redio.children(".uploadResultFE:checked").val()=="Yes"){
        ...
        if(obj.fe_json!="" & !obj.fe_json.includes('"default_flag":1')){
            try{
            ...
            try{
                ...
                //var temp_bus=sensor_form.eq(0).children("select[name='sensorBus']").val();
                
                //var temp_channel_name = sensor_form.eq(0).children("select[name='sensorChName']").val();
