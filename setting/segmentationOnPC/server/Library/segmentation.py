...
class segRule(multiprocessing.Process):
    ...
    def seg_using_STFT_freq(self, signal, fs, distance, height, sig_threshold, time_range, sx_energy_threshold, certain_freq):
        ...
        #if np.max(Sx[:, peak_idx]) > sx_energy_threshold:
        #    if np.max(signal.abs()) > sig_threshold:
                ...
                if targetObj != None:
                    ...
                    for d in testFreqData:
                        ...
                        if len(d['data'])<=int(targetObj['fs']/5):
                            continue
                        t,f,f_id,_ = self.get_max_freq(d['data'],targetObj['fs'],full=True)
                        ...
                        f_max = np.array(f[f_id],dtype=np.int32)
                        t_freq = t+d['time']-minTime
                        freq_counts = pd.Series(f_max).value_counts()
                        f_peaks = int(freq_counts.index[0])
                        if f_peaks == 0:
                            f_peaks = int(freq_counts.index[1])
                        ...
                        equal_start_idx = t_freq[f_max == f_peaks][0]
                        equal_end_idx = t_freq[f_max == f_peaks][-1]
                        peak_idx = (equal_start_idx+equal_end_idx)/2
                    ...
                    data_pd = pd.DataFrame()
                    data_pd[basis_column] = targetObj["data"][basis_column]
                    columeMaxLen = len(targetObj["data"][basis_column])
                    minScale = 60000
                    if columeMaxLen>minScale:
                        lastTime = 0
                        timeR = np.array([])
                        dataR = np.array([])
                        i=0
                        while i < columeMaxLen:
                            print("STFT batch",i,"/",columeMaxLen)
                            if(len(data_pd[basis_column][i:i+minScale])<targetObj['fs']//5):
                                break
                            t,f,f_id,_ = self.get_max_freq(data_pd[basis_column][i:i+minScale],targetObj['fs'],full=True)
                            timeR = np.append(timeR,t+lastTime)
                            dataR = np.append(dataR,f[f_id])
                            lastTime = max(timeR)
                            
                            i=i+minScale
                    else:
                        t,f,f_id,_ = self.get_max_freq(data_pd[basis_column],targetObj['fs'],full=True)
                        timeR=t
                        dataR=f[f_id]
                    """
                    draw=dict()
