...
executable_list=("${executable_path[0]}/main" "${executable_path[1]}/statusServer" "${executable_path[1]}/server" "${executable_path[2]}/wifi_search" "${executable_path[2]}/reset" "${executable_path[2]}/start_NTP" "${executable_path[3]}/setup")
...
do
	...
	sudo gcc -Os -I /usr/include/python3.9 -o "${executable_list[i]}" "${executable_list[i]}.c" -lpython3.9 -lpthread -lm -lutil -ldl
