...
FORMAT = pyaudio.paFloat32 #or using paFloat32 
...
class Microphone(object): # create microphone object
    ...
    def open_mic_stream(self): #turn on the microphone
        print("self.device_index",self.device_index,type(self.device_index),CHANNELS)
...
def recording(filename, microphone_obj):
    ...
    print ("* recording")
    recording_data = np.zeros(RECORD_SECONDS*RATE+INPUT_FRAMES_PER_BLOCK, dtype=np.float32)
    for i in range(0, int(RATE / INPUT_FRAMES_PER_BLOCK * RECORD_SECONDS)+1):
        ...
        recording_data[i*INPUT_FRAMES_PER_BLOCK:(i+1)*INPUT_FRAMES_PER_BLOCK] = np.frombuffer(data,dtype = np.float32)
        
    print ("* done recording")
    microphone_obj.stop()
    microphone_obj.pa.terminate()
    #save_as_wav(filename, microphone_obj,recording_data)
...
if __name__ == "__main__":
    ...
    for i in range(len(microphone_index)):
        microphone_obj[i] = Microphone(microphone_index[i],microphone_obj[i])
