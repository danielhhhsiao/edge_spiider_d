...
RECORD_SECONDS = 30 #recording time
...
def recording(filename, microphone_obj):
    ...
    print(save_in_buffer(microphone_obj,recording_data[0:RECORD_SECONDS*RATE*CHANNELS]))
...
def save_in_buffer(microphone_obj, data):
    with io.BytesIO() as audio_buffer :
        with wave.open(audio_buffer,'wb') as wf:
            ...
            wf.setframerate(RATE)
            wf.writeframes(data)
        return len(audio_buffer.getvalue())
...
if __name__ == "__main__":
	...
	#index_offset iS used to point to the correct device
    print('-------------------')
    ...
    ch_loc = ['usb2','usb1','usb3'] # load from config 
    ch_name = ['microphone1','microphone2','microphone3'] #load from config
    for i in range(4):
        try:
            usb_index = audio_cards.index(ch_loc[i]) + index_offset
            microphone_index.append(usb_index)
            microphone_obj.append(ch_loc[i])
            audio_cards.pop(audio_cards.index(ch_loc[i]))
        except:
            pass
    
    print(microphone_index)
    print(microphone_obj)    
    a = input('STOP HERE')
    ...
    #print(microphone_obj[0].pa.get_device_count())
    #print(microphone_obj[1].pa.get_device_count())
    #for i in range (0,microphone_obj[0].pa.get_device_count()):
    #    print ("Device id ", i, " - ", microphone_obj[0].pa.get_device_info_by_host_api_device_index(0,i))
    #os.system('clear')
