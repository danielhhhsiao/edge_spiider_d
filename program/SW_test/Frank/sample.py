...
class sample(multiprocessing.Process):
    def __init__(self,param=dict(),minProcTime=0.01,simulator=0,oldData=False):
        ...
        vibMinLen = min([len(vibLocation),len(vibFs),len(vibScale),len(vibName)])
...
class vibration(multiprocessing.Process):
    def __init__(self,bus,name,Fs,Scale,minProcTime):
        ...
        if hubType!="Vibration board":
            ...
            glovar.sensorOrder = [0,0,0,0,0,0]
            glovar.maxEnable = 0
            for i,n in enumerate(self.nameStr):
                if n =="":
                    ...
                    select.append(True)
                    glovar.sensorOrder[glovar.maxEnable] = sensorType[i]
                    glovar.maxEnable = glovar.maxEnable + 1
    ...
    def createDataObj(self,startTime,endTime,data):
        ...
        for i in range(0,data.shape[0],3):
            if(glovar.sensorOrder[counter] == 1):
                data[i] = data[i]/32768.0
                data[i+1] = data[i+1]/32768.0
                data[i+2] = data[i+2]/32768.0
            elif (glovar.sensorOrder[counter] == 2):
                data[i] = data[i]/512000.0
                data[i+1] = data[i+1]/512000.0
                data[i+2] = data[i+2]/512000.0
            elif (glovar.sensorOrder[counter] == 3):
                data[i] = data[i]/32768.0
                data[i+1] = data[i+1]/32768.0
                data[i+2] = data[i+2]/32768.0
                
            counter = counter+1
            counter = counter%glovar.maxEnable
        ...
        for i,n in enumerate(self.enableName):
            if not self.sensorErrFlag[i]:
                ...
                x = np.array(data[i*3])*scale
                y = np.array(data[i*3+1])*scale
                z = np.array(data[i*3+2])*scale
...
class modbus_process(multiprocessing.Process):
    def __init__(self,maxUSBLen,connection,setting,minProcTime):
        ...
        for k in byAddress.keys():
            ...
            for date in byAddress[k]:
                ...
                if date['register']-1!=lastRegister:
                    d = dict()
                    ...
                    d["delay"]=date['delay']
                    d["check"]=date['check']
                    d["dataReciveTime"]= 5 #address 1 + function code 1 + length 1 + CRC 2
                    d["count"]=1
                    d["obj"]=[]
                    d["obj"].append(obj)
                    self.batch.append(d)
                ...
                self.batch[-1]["dataReciveTime"] +=2 # data 2
    ...
    def getModbusArr(self,slave_address,function_code,register,register_count):
        ...
        hi,lo = self.CRC_16(arr)
        arr.append(lo)
        arr.append(hi)
        return arr
        
    def checkLen(self,count,arr):
        print("checkLen",len(arr),count)
        return  len(arr)==count*2+5 #address 1 + function code 1 + length 1 + CRC 2
    ...
    def createBuffer(self):
        ...
        for k in self.nameAndRule.keys():
            ret[k] = np.zeros(self.maxSamplePoint*2,dtype=np.int8)
        return ret , timeArr
        ...
        while not self.states["release"]:
            ...
            #try:
                ...
                for b in self.batch:
                    ...
                    while self.port.out_waiting>0 and time.time()-timeLimitStart<self.period:
                        time.sleep(0.01)
                        
                    if time.time()-timeLimitStart>self.period:
                        raise Exception("transmission timeout")
                    
                    #response + receive time
                    time.sleep(b["delay"]/1000+self.oneByteTime*b["dataReciveTime"])
                    
                    lastLan = -1
                    timeLimitStart = time.time()
                    while (lastLan!=self.port.in_waiting or self.port.in_waiting<5) and time.time()-timeLimitStart<self.period:
                        lastLan = self.port.in_waiting
                        time.sleep(0.01)
                    ...
                    useOldDate = False
                    if not self.checkLen(b["count"],rec) :
                        useOldDate = True
                    ...
                    if useOldDate: 
                        if bufferIndex>0:
                            for k in b["obj"]:
                                for n in k["name"]:
                                    bufferTotalArr[n][bufferIndex*2] = bufferTotalArr[n][bufferIndex-1]
                    else:
                        registerDate = self.getModbusArrDate(rec)
                        for i,k in enumerate(b["obj"]):
                            if len(k["name"])==1:
                                bufferTotalArr[k["name"][0]][bufferIndex*2]=registerDate[i][0]
                                bufferTotalArr[k["name"][0]][bufferIndex*2+1]=registerDate[i][1]
                            else:
                                byte2 = registerDate[i][0]<<8 | registerDate[i][1]
                                mask = 0x1
                                for n in k["name"]:
                                    if n != "":
                                        bufferTotalArr[n][bufferIndex*2+1] = mask & byte2
                                    mask <<= 1
            ...
            if newSampleTime-peroidTime>=self.minProcTime and bufferIndex>0:
                for k in bufferTotalArr.keys():
                    bufferTotalArr[k] = bufferTotalArr[k][:bufferIndex*2]
    ...
    def createDataObj(self,startTime,endTime,data):
        ...
        for k in data.keys():
            data[k] = np.array(data[k],dtype=np.uint8) # translate to numpy
            if self.nameAndRule[k]['type']=="bit":
                data[k].dtype = np.int16 #2byte to 1 int
                data[k] = np.array(data[k],dtype=np.uint8) #compress memory size
            elif self.nameAndRule[k]['type']=="float32":
                data[k].dtype = np.float32 #4byte to 1 float
                data[k] = np.array(data[k],dtype=np.uint8) #translate to float
            elif self.nameAndRule[k]['type']=="int16":
                data[k].dtype = np.int16 #2byte to 1 int
            elif self.nameAndRule[k]['type']=="uint16":
                data[k].dtype = np.uint16 #2byte to 1 uint
            elif self.nameAndRule[k]['type']=="BCD":
                data[k].dtype = np.uint16 #2byte to 1 uint
                newData = map(lambda x:int(hex(x).replace("0x","")),data[k])
...
class prognosis(multiprocessing.Process):
    def __init__(self,port,portNum,maxUSBLen,name,Fs,Scale,minProcTime):
        multiprocessing.Process.__init__(self)
        self.exit = multiprocessing.Event()
        print("Prognosis class create")
             
        self.config = dict()
        self.config["defaultBaudrate"]              =921600
        ...
        self.offsetRatioForTx=1/8*10/self.config["defaultBaudrate"]
        self.maxUSBLen=maxUSBLen
        self.usbPort=port
        ...
        if self.usbPort==None:
            ...
            self.port = serial.Serial(self.usbPort,self.config["defaultBaudrate"],parity=serial.PARITY_NONE, bytesize=8 )
            ...
            if(self.config[self.sensor]["stdCommand"] not in rdata ):
                if self.port_scan_baud() == None :
                    ...
                    self.set_sensor_baud(self.config["defaultBaudrate"])
                    rdata=self.get_infor()
                    if(self.config[self.sensor]["stdCommand"] not in rdata):
        ...
        self.minSamplePeriod = min(self.config[self.sensor]["oneBatchSample"]/self.Fs,0.01)
    ...
    def getBuffer(self,arr):
        ...
        #bufferStartTime -= 0.18
        bufferStartTime -= 0.045
    ...
    def run(self):
        ...
        try:
            ...
            while not self.states["release"]:
                sleepTime = self.minSamplePeriod-(time.time()-newSampleTime)
                #print("sleepTime",sleepTime,self.minSamplePeriod,self.portNum)
                if(sleepTime>0):
                    time.sleep(sleepTime)
                newSampleTime=time.time()
                ...
                #print(self.port.in_waiting,time.time()-testTime)
                serialBuffer,serialBufferStartTime = self.getBuffer(serialBuffer)
                #print(self.port.in_waiting,
                #self.config[self.sensor]["oneSampleByte"],
                #bufferIndex+self.config[self.sensor]["oneBatchSample"],
                #self.bufferTotalArrSize)
                while(len(serialBuffer) >= self.config[self.sensor]["oneSampleByte"]
                    and bufferIndex+oneSampleByte<= self.bufferTotalArrSize
                    and zeroCount==0):
                    
                    data = serialBuffer[:self.config[self.sensor]["oneSampleByte"]]
                    serialBuffer = serialBuffer[self.config[self.sensor]["oneSampleByte"]:]
                    dataStartTime = serialBufferStartTime
                    serialBufferStartTime += self.config[self.sensor]["oneBatchSample"]/self.realFs
                    #print(data[0:3])
                    if data[0:3]!=b'AT3':
                        zeroCount=401
                ...
                if freshFsCount/self.Fs>3:
                    ...
                    freshFsEnd = serialBufferStartTime
                    newFs = freshFsCount/(freshFsEnd-freshFsStart)
                    self.realFs = self.realFs*0.9+newFs*0.1
                    print("Fs=",(freshFsEnd-freshFsStart),freshFsCount,newFs,self.realFs)
                    freshFsCount=0
