...
if __name__ == "__main__":
        ...
        sensorOrder = [0,0,0,0,0,0]
        maxEnable = 0
        logging.info("Enable sensor at device...")
        for i,enableSensor in enumerate(args.ch):
                if enableSensor=="1":
                        if sensors[i]:
                                sensorOrder[maxEnable] = sensorType[i]
                                maxEnable = maxEnable + 1
        ...
        resultArr = resultArr.reshape(-1,1) 
        for i in range(0,resultArr.shape[0],1):
            if ((resultArr[i] & 0x800000) != 0):
                resultArr[i] = (resultArr[i]+0xFF000000)
        resultArr = resultArr /16
        #----------------------------------------------------------
        counter = 0
        if not args.raw:
            for i in range(0,resultArr.shape[0],3):
                if(sensorOrder[counter] == 1):
                    resultArr[i] = resultArr[i]/32768.0*1000
                    resultArr[i+1] = resultArr[i+1]/32768.0*1000
                    resultArr[i+2] = resultArr[i+2]/32768.0*1000
                elif (sensorOrder[counter] == 2):
                    resultArr[i] = resultArr[i]/512000.0*1000
                    resultArr[i+1] = resultArr[i+1]/512000.0*1000
                    resultArr[i+2] = resultArr[i+2]/512000.0*1000
                elif (sensorOrder[counter] == 3):
                    resultArr[i] = resultArr[i]/32768.0*1000
                    resultArr[i+1] = resultArr[i+1]/32768.0*1000
                    resultArr[i+2] = resultArr[i+2]/32768.0*1000
                counter = counter+1
                counter = counter%maxEnable
        #----------------------------------------------------------
