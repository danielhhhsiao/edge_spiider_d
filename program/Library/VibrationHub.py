...
#1.1.0 support v1.1.1 of vibration FW (MPU/ADXL/ICM)

    __version__ = "1.1.0"
    def __init__(self,spiChannel):
        
        self.typeMappingScale        = dict()
        self.typeMappingScale[1]     = 1.0/16.0/32768.0   #20bit to 16bit MPU9250/6500
        self.typeMappingScale[2]     = 1.0/16.0/512000.0  #ADXL355 remove 4 bit (the 4 bit are sample status)
        self.typeMappingScale[3]     = 1.0/16.0/32768.0   #20bit to 16bit ICM42605
...
if __name__ == "__main__":
        ...
        logging.info("Software version: "+VibHub.__version__)
        ...
        parser.add_argument("fs",type=int,help="Sampling rate. Example 1000, 4000 or 8000."
                ,choices=[1000,4000,8000],nargs='?',default = 4000)
        ...
        if args.cs !=0 and args.cs !=1:
                ...
                sys.exit()
        if args.fs !=1000 and args.fs !=4000 and args.fs != 8000:
        ...
        fileName = "Vib_result_"+startTimeStr
        ...
        if not vib.CheckSlave():
                ...
                logging.info("Board Version: "+vib.GetFWVersion())
        ...
        #----------------------------------------------------------
        logging.info("Scan the type sensor...")       
        sensorType = vib.GetType()
        for i in range(0,6,1):
            if(sensorType[i] == 1):
                logging.info("\tthe device type of channel %d is MPU sensor."%(i+1))
            elif(sensorType[i] == 2):
                logging.info("\tthe device type of channel %d is ADXL sensor."%(i+1))
            elif(sensorType[i] == 3):
                logging.info("\tthe device type of channel %d is ICM sensor."%(i+1))
            else:
                logging.info("\tthe device type of channel %d is un-know."%(i+1))
        #----------------------------------------------------------
        ...
        for i,enableSensor in enumerate(args.ch):
                if enableSensor=="1":
                        if sensors[i]:
                                sensorOrder.append(sensorType[i])
        ...
        perSampleLen = maxEnableSensor*9
        ...
        #----------------------------------------------------------
        resultArr = resultArr[0::3]*65536 + resultArr[1::3]*256 + resultArr[2::3]
        resultArr = vib.ConvertData(resultArr)
        negativeIndex = (resultArr & 0x800000) != 0
        resultArr[negativeIndex] = (resultArr[negativeIndex]+0xFF000000)
        ...
        for i in range(0,resultArr.shape[0],3):
                ...
                z= resultArr[i+2]
                if not args.raw:
                        x= x*columnScale[i//3]*vib.typeMappingScale[sensorOrder[i//3]]*1000
                        y= y*columnScale[i//3]*vib.typeMappingScale[sensorOrder[i//3]]*1000
                        z= z*columnScale[i//3]*vib.typeMappingScale[sensorOrder[i//3]]*1000
                df[columnName[i//3]+"_aX"]=x
                df[columnName[i//3]+"_aY"]=y
                df[columnName[i//3]+"_aZ"]=z
                
        #print("Processe timing:",time.time()-startTestingTime)
